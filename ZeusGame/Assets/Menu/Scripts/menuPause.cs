...
public class menuPause : MonoBehaviour
{
	...
	void Update (){
		...
		if (IsGamePaused () && (Input.GetKeyDown ("escape") || Input.GetKeyDown ("return") || Input.GetKeyDown ("joystick button 7"))) {
	...
	public void PauseGame(bool check) {
		//print ("x: " + playerCamera.sensitivityX);
		//print ("y: " + mainCamera.sensitivityY);
		if(Input.GetJoystickNames ().Length == 0){
			Cursor.visible = true;
		}
		...
		//GetComponent<HeadBob> ().enabled = false; Edited out since component not used
		GetComponent<guiCrosshair> ().enabled = false;
		GetComponent<guiPrompt> ().enabled = false;
		FindObjectOfType<guiPrompt>().hidePrompt (); //If there is a prompt active remove it from the screne
