...
//Modified by Jordan Hoffman to make setVelocity a public method
using UnityEngine;
...
public class CharacterMotor : MonoBehaviour
{
    ...
    public Vector3 LocalizedInput
    {
        get
        {
            return tr.InverseTransformDirection (inputMoveDirection);
        }
    }
    ...
    public class CharacterMotorMovement
    {
        ...
        public float maxSprintAcceleration = 7.0f;
        public float maxAirAcceleration = 5.0f;
    ...
    {
        // Find desired velocity
        float maxSpeed = MaxSpeedInDirection(LocalizedInput);
        if(grounded)
        {
        ...
        return tr.TransformDirection (LocalizedInput * maxSpeed);
    }
    ...
    float GetMaxAcceleration(bool grounded)
    {
        ...
        if (grounded)
        {
            if ((inputSprint && movement.velocity.magnitude >= movement.maxForwardSpeed) || movement.velocity.magnitude > movement.maxForwardSpeed)
            {
                // print ("sprint accel");
                return movement.maxSprintAcceleration;
            }
            else
            {
                // print ("ground accel");
    ...
    float MaxSpeedInDirection(Vector3 desiredMovementDirection)
    {
        ...
        else
        {
            ...
            if (desiredMovementDirection.z > 0)
            {
                if (inputSprint) zAxisEllipseMultiplier = movement.maxSprintSpeed;
                else zAxisEllipseMultiplier = movement.maxForwardSpeed;
            }
            else zAxisEllipseMultiplier = movement.maxBackwardsSpeed;
            zAxisEllipseMultiplier /= movement.maxSidewaysSpeed;
    ...
    {
        return (inputSprint && LocalizedInput.z > Mathf.Abs (LocalizedInput.x) && movement.velocity.magnitude > movement.maxForwardSpeed);
    }

    public bool IsWalking ()
    {
        return movement.velocity.magnitude > 0;
    }
}